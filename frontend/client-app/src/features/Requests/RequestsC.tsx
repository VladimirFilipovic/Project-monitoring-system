...
import { Button, Container, FormField, Header, Input, Label, Segment } from "semantic-ui-react";
...
export default function RequestsC({closeForm}: Props) {
    ...
    const validationSchema = Yup.object({
        ...
        requestDate: Yup.date().required('The deadline date is required').min(new Date(Date.now()+46400000),"Request Date must be later than " + new Date().toUTCString()),
   ...
   console.log(reqCopy)
    ...
    // function onFileInputChangeHandler(event: any) {
    ...
    //                 setRequest({...request,specification: fileData})
    //                 console.log(request)
    //             }
    //         }
    //     }
...
<           Segment clearing>
                ...
                <Formik validationSchema={validationSchema} enableReinitialize initialValues= {request}  onSubmit={handleCreateRequest}>
                    {({handleSubmit, resetForm}) => (
                        <Form className='ui form' onSubmit={handleSubmit} autoComplete='off'>
                            <FormField>
                                <Field placeholder='Name' name='name'/>
                                <ErrorMessage name='name' render={error => <Label basic color='red' content={error}/>} />
                            </FormField>
                            <FormField>
                            <Field type='date' placeholder='Deadline date' name='requestDate'/>
                                <ErrorMessage name='requestDate' render={error => <Label basic color='red' content={error}/>} />
                            </FormField>
                            <FormField>
                            <Input  onChange={changeHandleFile} type="file" name="specification" />
                            </FormField>
                             <Button floated='right' positive type='submit' content='Submit'/>
                             <Button floated='right'  type="button" onClick={() => resetForm()} content='Clear' />
                        </Form>
                    )}
                </Formik>
            </Segment>
